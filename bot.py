...
async def play_episode(update: Update, context: ContextTypes.DEFAULT_TYPE,
    ...
    if config.WEBAPP_URL:
        import base64
        import urllib.parse
        encoded = base64.urlsafe_b64encode(http_link.encode()).decode().rstrip("=")
        title_enc = urllib.parse.quote(movie.title or "")
        ep_enc = urllib.parse.quote(ep.label or "")
        webapp_url = f"{config.WEBAPP_URL}/?t={title_enc}&e={ep_enc}#{encoded}"
